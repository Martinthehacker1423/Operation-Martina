...
Internal assessment system. For authorized use only.
"""
...
def _load_config() -> dict:
    ...
    cfg = {
        "access_code":       "AURORA-REDLINE-7749",
        "admin_key":         "ASTRA-CONTROL-9999",
        "candidate_name":    "Martina",
...
def _init_db():
    with _db() as c:
        ...
        c.execute("INSERT OR IGNORE INTO trial (name, access_code) VALUES (?,?)",
                  (CONFIG["candidate_name"], CONFIG["access_code"]))
        row = c.execute("SELECT id FROM trial WHERE access_code=?",
                        (CONFIG["access_code"],)).fetchone()
        if row:
            tid = row["id"]
            for pn in range(1, 5):
                c.execute("INSERT OR IGNORE INTO phases (trial_id, phase_num) VALUES (?,?)",
                          (tid, pn))
...
def _get_trial():
    with _db() as c:
        return c.execute("SELECT * FROM trial WHERE access_code=?",
                         (CONFIG["access_code"],)).fetchone()

def _get_phase(trial_id: int, phase_num: int):
    with _db() as c:
        return c.execute("SELECT * FROM phases WHERE trial_id=? AND phase_num=?",
                         (trial_id, phase_num)).fetchone()

def _time_left(start_time: float) -> float:
    """Seconds remaining in 30-day trial."""
    elapsed = time.time() - start_time
    return max(0.0, TRIAL_DURATION_DAYS * 86400 - elapsed)

def _phase_time_left(sub_deadline: Optional[float]) -> Optional[float]:
    if sub_deadline is None:
        return None
    return max(0.0, sub_deadline - time.time())
...
def _check_expired(trial) -> bool:
    if not trial["start_time"]:
        return False
    return _time_left(trial["start_time"]) <= 0
...
def _generate_report(trial_id: int) -> tuple[str, bool]:
    """Generate final report from all 4 phase evaluations. Returns (report_text, overall_passed)."""
    results = []
    all_passed = True
    with _db() as c:
        phases = c.execute("SELECT * FROM phases WHERE trial_id=? ORDER BY phase_num",
                           (trial_id,)).fetchall()
    for ph in phases:
        pn  = ph["phase_num"]
        raw = ph["eval_raw"]
        ...
        results.append({"phase": pn, "title": PHASES[pn]["title"], "score": score,
                         "passed": passed, "summary": ev.get("feedback_summary", "—")})
        if not passed:
            all_passed = False
    ...
    user_msg = (
        ...
        f"Trial: AutoPurple Executive Qualification\n"
        f"Overall result: {'PASSED' if all_passed else 'NOT PASSED'}\n\n"
        f"Phase scores:\n{scores_block}\n\n"
        f"Phase-by-phase summaries:\n{detail_block}\n\n"
...
def _gen_credential() -> str:
    ...
    h    = hashlib.sha256(f"{CONFIG['access_code']}{ts}{rand}".encode()).hexdigest()[:8].upper()
    return f"AP-EXEC-{ts}-{rand}-{h}"

def _get_dialog_transcript(trial_id: int, phase_num: int) -> str:
    with _db() as c:
        rows = c.execute(
            "SELECT role, content FROM messages WHERE trial_id=? AND phase_num=? ORDER BY ts",
            (trial_id, phase_num)
        ).fetchall()
    lines = []
    for r in rows:
...
def login(req: LoginReq):
    ...
    if code == CONFIG["access_code"].upper():
        return {"role": "candidate", "name": CONFIG["candidate_name"]}
    if code == CONFIG["admin_key"].upper():
        return {"role": "admin"}
    raise HTTPException(401, "Invalid access code")

# ── Status ─────────────────────────────────────────────────────────────────────
@app.get("/api/status")
def status(role: str = "candidate"):
    trial = _get_trial()
    if not trial:
        raise HTTPException(500, "Trial record missing")
    ...
    phases_out = []
    with _db() as c:
        all_phases = c.execute("SELECT * FROM phases WHERE trial_id=? ORDER BY phase_num",
                               (tid,)).fetchall()
    for ph in all_phases:
        pn     = ph["phase_num"]
        ptime  = _phase_time_left(ph["sub_deadline"]) if ph["sub_deadline"] else None
    ...
    # check astra messages
    with _db() as c:
        astra_msgs = c.execute(
            "SELECT content FROM messages WHERE trial_id=? AND role='astra' ORDER BY ts",
            (tid,)
        ).fetchall()

    report = None
    with _db() as c:
        rep = c.execute("SELECT * FROM report WHERE trial_id=?", (tid,)).fetchone()
    if rep:
...
def start_trial():
    ...
    now = time.time()
    with _db() as c:
        c.execute("UPDATE trial SET start_time=?, status='active' WHERE id=?",
                  (now, trial["id"]))
        # Unlock Phase 1 immediately with 72h sub-timer
        deadline_p1 = now + PHASE1_SUB_HOURS * 3600
        c.execute("""UPDATE phases SET status='active', unlocked_at=?, sub_deadline=?
                     WHERE trial_id=? AND phase_num=1""",
                  (now, deadline_p1, trial["id"]))
        # Store Astra start message
        c.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,0,'astra',?)",
                  (trial["id"], ASTRA["trial_start"]))

    return {"ok": True, "message": "Trial started. Phase 1 is now active."}
...
def phase_detail(phase_num: int):
    ...
    with _db() as c:
        msgs = c.execute(
            "SELECT role, content, ts FROM messages WHERE trial_id=? AND phase_num=? AND role != 'astra' ORDER BY ts",
            (trial["id"], phase_num)
        ).fetchall()
...
def submit_phase(phase_num: int, req: SubmitReq):
    ...
    if ph["status"] not in ("active", "failed"):
        raise HTTPException(400, f"Phase {phase_num} cannot be submitted in status '{ph['status']}'")

    # Check sub-deadline
    if ph["sub_deadline"] and time.time() > ph["sub_deadline"]:
        with _db() as c:
            c.execute("UPDATE phases SET status='failed' WHERE trial_id=? AND phase_num=?",
                      (tid, phase_num))
        raise HTTPException(400, "Phase sub-deadline has passed")

    # Get dialog transcript for phases that use it
    transcript = ""
    if PHASES[phase_num]["has_dialog"]:
        transcript = _get_dialog_transcript(tid, phase_num)

    now = time.time()
    with _db() as c:
        c.execute("""UPDATE phases SET status='evaluating', submitted_at=?, submission_text=?
                     WHERE trial_id=? AND phase_num=?""",
    ...
    try:
        ...
        with _db() as c:
            c.execute("UPDATE phases SET status='active' WHERE trial_id=? AND phase_num=?",
                      (tid, phase_num))
    ...
    with _db() as c:
        c.execute("""UPDATE phases SET status=?, score=?, feedback=?, eval_raw=?
                     WHERE trial_id=? AND phase_num=?""",
                  (new_status, score, ev.get("feedback_summary", ""), json.dumps(ev),
                   tid, phase_num))

        # Astra message after Phase 1
        if phase_num == 1:
            c.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,1,'astra',?)",
                      (tid, ASTRA["phase1_complete"]))
        ...
        if passed and phase_num < 4:
            ...
            # Astra message on Phase 3 unlock
            if next_pn == 3:
                c.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,3,'astra',?)",
                          (tid, ASTRA["phase3_unlock"]))

        # Check if all phases complete → generate final report
        if passed and phase_num == 4:
            _finalize_trial(tid, c)

    return {"ok": True, "score": score, "passed": passed, "feedback": ev.get("feedback_summary", "")}

def _finalize_trial(trial_id: int, conn: sqlite3.Connection):
    try:
        report_text, all_passed = _generate_report(trial_id)
    except Exception:
        report_text  = "Report generation encountered an error. Please contact AutoPurple."
        all_passed   = False

    credential = _gen_credential() if all_passed else None
    conn.execute("UPDATE trial SET status=? WHERE id=?",
                 ("complete" if all_passed else "failed", trial_id))
    conn.execute(
        "INSERT OR REPLACE INTO report (trial_id, text, credential, passed, generated_at) VALUES (?,?,?,?,?)",
        (trial_id, report_text, credential, int(all_passed), time.time())
    )
    astra_msg = (ASTRA["trial_passed"].format(credential=credential) if all_passed
                 else ASTRA["trial_failed"])
    conn.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,4,'astra',?)",
                 (trial_id, astra_msg))

# ── Chat (dialogue phases) ─────────────────────────────────────────────────────
...
def chat(phase_num: int, req: ChatReq):
    if phase_num not in PHASES or not PHASES[phase_num]["has_dialog"]:
        raise HTTPException(400, "This phase does not have a dialogue")
    trial = _get_trial()
    tid   = trial["id"]
    if _check_expired(trial):
        raise HTTPException(400, "Trial has expired")
    ph = _get_phase(tid, phase_num)
    if ph["status"] not in ("active",):
        raise HTTPException(400, f"Phase {phase_num} is not active (status: {ph['status']})")

    # Load history
    with _db() as c:
        rows = c.execute(
            "SELECT role, content FROM messages WHERE trial_id=? AND phase_num=? AND role!='astra' ORDER BY ts",
            (tid, phase_num)
        ).fetchall()
    ...
    reply = _claude(
        messages=history,
        system=PHASES[phase_num]["dialog_system"],
        max_tokens=600,
    )

    with _db() as c:
        c.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,?,?,?)",
                  (tid, phase_num, "user", req.message))
        c.execute("INSERT INTO messages (trial_id, phase_num, role, content) VALUES (?,?,?,?)",
                  (tid, phase_num, "assistant", reply))

    return {"reply": reply}

# ── Astra messages ─────────────────────────────────────────────────────────────
@app.get("/api/astra")
def get_astra_messages():
    trial = _get_trial()
    with _db() as c:
        rows = c.execute(
            "SELECT phase_num, content, ts FROM messages WHERE trial_id=? AND role='astra' ORDER BY ts",
            (trial["id"],)
        ).fetchall()
...
def admin_view(key: str = ""):
    if key.upper() != CONFIG["admin_key"].upper():
        raise HTTPException(403, "Unauthorized")
    trial = _get_trial()
    tid   = trial["id"]
    with _db() as c:
        phases = c.execute("SELECT * FROM phases WHERE trial_id=? ORDER BY phase_num", (tid,)).fetchall()
        msgs   = c.execute("SELECT * FROM messages WHERE trial_id=? ORDER BY ts", (tid,)).fetchall()
        rep    = c.execute("SELECT * FROM report WHERE trial_id=?", (tid,)).fetchone()
    ...
    return {
        ...
        "phases":     phases_out,
        "messages":   [{"phase": m["phase_num"], "role": m["role"], "content": m["content"],
...
def admin_override(phase_num: int, passed: bool, key: str = ""):
    ...
    new_status = "passed" if passed else "failed"
    with _db() as c:
        c.execute("UPDATE phases SET status=?, score=? WHERE trial_id=? AND phase_num=?",
                  (new_status, 100 if passed else 0, tid, phase_num))
        if passed and phase_num < 4:
            next_pn  = phase_num + 1
            deadline = (time.time() + PHASE4_SUB_HOURS * 3600) if next_pn == 4 else None
            c.execute("UPDATE phases SET status='active', unlocked_at=?, sub_deadline=? WHERE trial_id=? AND phase_num=?",
                      (time.time(), deadline, tid, next_pn))
        if passed and phase_num == 4:
            _finalize_trial(tid, c)
    return {"ok": True}

# ── Config API (admin GUI) ─────────────────────────────────────────────────────
@app.get("/api/config")
...
def update_config(req: ConfigUpdateReq, key: str = ""):
    ...
    if req.anthropic_api_key is not None:
        CONFIG["anthropic_api_key"] = req.anthropic_api_key.strip()
    if req.access_code is not None and req.access_code.strip():
        CONFIG["access_code"] = req.access_code.strip().upper()
    if req.candidate_name is not None and req.candidate_name.strip():
        CONFIG["candidate_name"] = req.candidate_name.strip()
    CONFIG_FILE.write_text(json.dumps(CONFIG, indent=2, ensure_ascii=False))
    # Re-sync DB candidate record
    with _db() as c:
        c.execute("UPDATE trial SET name=?, access_code=? WHERE id=1",
                  (CONFIG["candidate_name"], CONFIG["access_code"]))
    return {"ok": True, "api_key_set": bool(CONFIG.get("anthropic_api_key", ""))}

# ── Run ────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    uvicorn.run("server:app", host="127.0.0.1", port=7749, reload=False)
